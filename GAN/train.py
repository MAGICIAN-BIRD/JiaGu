...
def train(opt, dataloader, generator, discriminator, optimizer_g, optimizer_d, criterion, fixed_noise, log_filename, visualizer, output_dir, feature_extractor):
    ...
    for epoch in range(opt.epochs):
        for i, (imgs, labels) in enumerate(tqdm(dataloader)):
            ...
            real_labels = torch.ones(opt.batch_size, 1, device=opt.device)
            fake_labels = torch.zeros(opt.batch_size, 1, device=opt.device)
            ...
            d_loss_real = criterion(outputs, real_labels)  # 目标和输出已经一致
            d_loss_real.backward()

            z = torch.randn(opt.batch_size, opt.z_dim, 1, 1, device=opt.device)  # 随机噪声
            fake_imgs = generator(z, features)  # 将特征和噪声输入生成器
            outputs = discriminator(fake_imgs.detach())
            d_loss_fake = criterion(outputs, fake_labels)  # 目标和输出已经一致
            ...
            g_loss = criterion(outputs, real_labels)  # 目标和输出已经一致
            g_loss.backward()
            optimizer_g.step()
            ...
            if total_iters % opt.log_freq == 0:
                ...
                # 使用 csv.DictWriter 而不是 pandas.DictWriter
                with open(log_filename, mode='a', newline='') as f:
                    writer = csv.DictWriter(f, fieldnames=['epoch', 'batch', 'D_loss', 'G_loss'])
                    writer.writeheader() if total_iters == opt.batch_size else None  # 写入表头（仅写一次）
